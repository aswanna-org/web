...
// Crisp Vector Emblem generator for institutions
function InstitutionBadgeLogo({ institution, index }: { institution: AgriInstitution; index: number }) {
  ...
  const isGov = institution.type === 'gov';
  const isPvt = institution.type === 'pvt';

  const badgeColor = isGov
    ? ['#006837', '#1b5e20', '#2e7d32', '#0f4d30', '#0a5c36'][index % 5]
    : isPvt
    ? ['#0e7490', '#0369a1', '#1d4ed8', '#1e40af'][index % 4]
    : ['#047857', '#0f766e', '#0284c7', '#4338ca'][index % 4];
...
export default function GovernmentInstitutions() {
  ...
  // Fetch government institutions from API with pagination, category filter & search
  useEffect(() => {
    let isMounted = true;
    setIsLoading(true);
    ...
    const queryUrl = `${API_BASE_URL}/institutions?type=gov&page=${currentPage}&limit=${PAGE_SIZE}${catParam}${searchParam}`;
    ...
    fetch(queryUrl)
      ...
      .catch((err) => {
        console.error('Failed to fetch government institutions from API:', err);
        if (isMounted) {
          setInstitutions([]);
          setTotalPages(1);
          setTotalCount(0);
  ...
  const categoryFilters = [
    { key: 'all', labelSi: 'සියල්ල', labelEn: 'All Institutions' },
    { key: 'ministry', labelSi: 'අමාත්‍යාංශ', labelEn: 'Ministries' },
    { key: 'department', labelSi: 'දෙපාර්තමේන්තු', labelEn: 'Departments' },
    { key: 'research', labelSi: 'පර්යේෂණ ආයතන', labelEn: 'Research Institutes' },
    { key: 'board', labelSi: 'මණ්ඩල හා සභා', labelEn: 'Statutory Boards' },
    { key: 'authority', labelSi: 'අධිකාරි සහ සංස්ථා', labelEn: 'Authorities & Corps' }
  ];

  const handlePageChange = (page: number) => {
    setCurrentPage(page);
    const element = document.getElementById('institutions-grid-section');
    if (element) {
  ...
  const pageTitle = isSinhala ? 'රාජ්‍ය ආයතන තොරතුරු කේන්ද්‍රය' : 'GOVERNMENT INSTITUTIONS';
  const pageSubtitle = isSinhala
    ? 'ශ්‍රී ලංකාවේ කෘෂිකර්මාන්තයට සම්බන්ධ අමාත්‍යාංශ, දෙපාර්තමේන්තු, පර්යේෂණ ආයතන සහ ව්‍යවස්ථාපිත මණ්ඩල වල නිල වෙබ් අඩවි සහ තොරතුරු ජාලය.'
    : 'Official portal connecting farmers, agribusinesses, and researchers with Sri Lankan Government Ministries, Departments, and Statutory Boards.';
  ...
  return (
    <div className="w-full min-h-screen bg-[#fbfdfa] font-roboto">
      ...
      <PageHero
        ...
        image="https://images.unsplash.com/photo-1577495508048-b635879837f1?w=1600&q=80"
          ...
          <div className="flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-4">
            ...
            <div>
              ...
              <p className="text-xs sm:text-sm text-gray-500 font-medium mt-1">
                {isSinhala
                  ...
                  : `Showing ${totalCount} official institutions (Page ${currentPage} of ${totalPages})`}
              </p>
            </div>
            ...
            <div className="relative w-full lg:max-w-md">
              ...
              <input
                ...
                placeholder={
                  isSinhala
                    ? 'නම, කෙටි නම (DOA, HARTI, CCB) හෝ සේවාව සොයන්න...'
                    : 'Search by name, acronym (DOA, CCB), or service...'
                }
      ...
      <section id="institutions-grid-section" className="w-full py-12">
          ...
          {isLoading ? (
            ...
            <div className="text-center py-20 bg-white rounded-3xl border border-gray-100 shadow-sm p-8 max-w-lg mx-auto">
              ...
              <h3 className="text-lg font-bold text-gray-800 mb-1">
                {isSinhala ? 'ආයතනයක් හමු නොවීය' : 'No institutions found'}
              </h3>
              <p className="text-xs text-gray-500 mb-6 leading-relaxed">
                {isSinhala
                  ? 'ඔබගේ සෙවුම් පදය වෙනස් කර නැවත උත්සාහ කරන්න.'
                  : 'Please check your search keyword or clear the active category filters.'}
              </p>
              <button
                onClick={() => {
                  setSearchQuery('');
              ...
              >
                {isSinhala ? 'සියලු ආයතන පෙන්වන්න' : 'Show All Institutions'}
              </button>
            </div>
          ) : (
            <>
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4 sm:gap-5">
                {institutions.map((item, index) => {
                  ...
                  return (
                    ...
                    >
                      {/* Top: Institution Logo on Organic Cloud Background */}
                      <div className="w-full aspect-square max-w-[135px] sm:max-w-[150px] flex items-center justify-center relative">
                        ...
                        <InstitutionBadgeLogo institution={item} index={index} />
                      </div>

                      {/* Category & Short Name Pill */}
                      ...
                      {/* Bottom Action: Visit Website Button / Details Button */}
      ...
      {/* ── Institution Contact Details Modal ── */}
      {selectedDetail && (
          ...
          <div className="bg-white w-full max-w-2xl rounded-[28px] shadow-2xl overflow-hidden border border-gray-100 flex flex-col">
            ...
            <div className="bg-gradient-to-r from-[#0f4d30] to-[#186a43] p-6 text-white flex items-start justify-between relative">
              <div className="flex items-center gap-4">
                <InstitutionBadgeLogo institution={selectedDetail} index={0} />
                <div>
                ...
                {selectedDetail.phone && (
                  <div className="flex items-center gap-2.5">
                    ...
                    <a
                      ...
                      className="text-[#006837] hover:underline font-bold"
                    >
                      {selectedDetail.phone}
                    </a>
                  </div>
                ...
                {selectedDetail.email && (
                  <div className="flex items-center gap-2.5">
                    ...
                    <a
                      ...
                      className="text-gray-700 hover:text-[#006837] hover:underline"
                    >
                      {selectedDetail.email}
                    </a>
                  </div>
